# -*- coding: utf-8 -*-
"""
Safe-refresh calibration against exact optimum.

This experiment evaluates LOCAL, SAFE005, SAFE010 and SAFE020 on small dynamic
weighted graphs. Exact MILP solutions are recomputed along the update streams,
so quality is measured directly against proven optimum. A refresh candidate is
accepted only when its objective value improves the maintained solution.

The script is self-contained and supports a quick self-test mode, task-level
checkpoints, automatic resume and atomic output writes.
...
OUTPUT_DIR = Path(__file__).resolve().parent / "safe_refresh_output"
...
QUICK_DYNAMIC_UPDATE_LOAD = 0.03
...
# ============================================================
# SAFE-REFRESH TASKS / CHECKPOINTS
# ============================================================

from dataclasses import dataclass


SAFE_REFRESH_FRACTIONS = {
    "SAFE005": 0.005,
    "SAFE010": 0.010,
    "SAFE020": 0.020,
}
...
def dynamic_checkpoint(task: DynamicTask) -> Path:
    return (
        OUTPUT_DIR
        / "safe_dynamic_checkpoints"
        / f"{task.name}.csv"
    )
...
def safe_refresh_candidate(
    ...
    """
    Run STRONG4 and accept it only when its objective value is strictly
    higher than the already-feasible locally maintained solution.

    Returns
    -------
    chosen_solution, elapsed_s, accepted, relative_gain
    """
    ...
    current_value = solution_weight(current, w)
    candidate_value = solution_weight(candidate, w)
    ...
    if candidate_value > current_value + 1e-12:
        ...
        return candidate, elapsed, True, gain

    return set(current), elapsed, False, 0.0
...
def run_dynamic_task(task: DynamicTask) -> str:
    ...
    seed_mix = (
        task.seed
        + 1000003 * task.n
        + 97 * sum(ord(c) for c in task.graph_type)
        + 193 * sum(ord(c) for c in task.weight_mode)
    ) % (2**32 - 1)
    ...
    edge_pool = DynamicEdgePool(G.edges())
    w = make_weights(G, task.weight_mode, rng)
    ...
    states = {"LOCAL": set(S0)}
    for name in SAFE_REFRESH_FRACTIONS:
        states[name] = set(S0)
    ...
    refresh_intervals = {
        ...
        for name, frac in SAFE_REFRESH_FRACTIONS.items()
    }
    ...
    for step in range(1, total_steps + 1):
        update_type, u, v = apply_random_update(
            G=G,
            edge_pool=edge_pool,
            rng=rng,
        )

        exact_value, exact_success, exact_time, exact_msg = exact_mw2ps(
            G=G,
            w=w,
        ...
        # ----------------------------------------------------
        # LOCAL: local maintenance only
        # ----------------------------------------------------
        before = set(states["LOCAL"])

        t0 = time.perf_counter()
        local_state, evals, recourse = local_dynamic_step(
            G=G,
            w=w,
            S=before,
            u=u,
        ...
        states["LOCAL"] = local_state

        local_value = solution_weight(local_state, w)
        ...
        row["LOCAL_value"] = local_value
        row["LOCAL_ratio_to_OPT"] = (
            local_value / exact_value
            if exact_success and exact_value > 0
            else math.nan
        )

        # ----------------------------------------------------
        # SAFE periodic refresh policies
        # ----------------------------------------------------
        for name, interval in refresh_intervals.items():
            before = set(states[name])

            # Always perform the cheap local maintenance first.
            t0 = time.perf_counter()
            maintained, evals, local_recourse = local_dynamic_step(
                G=G,
                w=w,
                S=before,
                u=u,
            ...
            if attempt:
                chosen, refresh_elapsed, accepted, refresh_gain = (
            ...
            total_elapsed = local_elapsed + refresh_elapsed
            total_recourse = len(before.symmetric_difference(chosen))

            states[name] = chosen

            value = solution_weight(chosen, w)

            row[f"{name}_time_s"] = total_elapsed
            row[f"{name}_evals"] = evals
            row[f"{name}_recourse"] = total_recourse
            row[f"{name}_refresh_attempt"] = int(attempt)
            row[f"{name}_refresh_accept"] = int(accepted)
            row[f"{name}_refresh_gain"] = refresh_gain
        ...
        # Exact feasibility audit every state at this small scale.
        for name, S in states.items():
            if not is_feasible_2packing(G, S):
                raise AssertionError(
                    f"{task.name}: step={step}, {name} infeasible."
                )
    ...
    return f"DONE {task.name} (steps={total_steps})"
...
def summarize() -> Tuple[Path, Path, Path]:
    files = sorted(
        (OUTPUT_DIR / "safe_dynamic_checkpoints").glob("*.csv")
    )

    if not files:
        raise RuntimeError("No safe-refresh checkpoints found.")
    ...
    all_path = OUTPUT_DIR / "safe_dynamic_all_results.csv"
    all_df.to_csv(all_path, index=False)

    methods = ["LOCAL", "SAFE005", "SAFE010", "SAFE020"]

    rows = []

    for name in methods:
        valid = all_df[
            all_df[f"{name}_ratio_to_OPT"].notna()
        ]

        ratios = valid[f"{name}_ratio_to_OPT"]

        attempts = int(
            all_df[f"{name}_refresh_attempt"].sum()
        )
        accepts = int(
            all_df[f"{name}_refresh_accept"].sum()
        )

        accepted_gains = all_df.loc[
            all_df[f"{name}_refresh_accept"] == 1,
        ...
        rows.append({
            ...
            "exact_success_rate": float(
                all_df["exact_success"].mean()
            ),
            "mean_ratio_to_OPT": float(ratios.mean()),
            "median_ratio_to_OPT": float(ratios.median()),
            "min_ratio_to_OPT": float(ratios.min()),
            "p01_ratio_to_OPT": float(ratios.quantile(0.01)),
            "p05_ratio_to_OPT": float(ratios.quantile(0.05)),
            ...
            "mean_time_ms": float(
                1000.0 * all_df[f"{name}_time_s"].mean()
            ),
            "mean_recourse": float(
                all_df[f"{name}_recourse"].mean()
            ),
            "refresh_attempts": attempts,
            "refresh_accepts": accepts,
            "refresh_accept_rate": (
                accepts / attempts
                if attempts > 0
                else 0.0
            ),
            "mean_accepted_relative_gain": (
                float(accepted_gains.mean())
                if len(accepted_gains) > 0
                else 0.0
            ),
            "max_accepted_relative_gain": (
                float(accepted_gains.max())
                if len(accepted_gains) > 0
                else 0.0
            ),
        })
    ...
    summary_path = OUTPUT_DIR / "safe_dynamic_summary.csv"
    summary.to_csv(summary_path, index=False)

    # Task-level summary is useful if a particular graph/weight family fails.
    task_rows = []

    keys = ["graph_type", "seed", "weight_mode", "m0"]

    for key_values, g in all_df.groupby(keys, sort=True):
        row = dict(zip(keys, key_values))

        for name in methods:
            ratios = g[f"{name}_ratio_to_OPT"]

            row[f"{name}_mean_ratio"] = float(ratios.mean())
            row[f"{name}_min_ratio"] = float(ratios.min())
            row[f"{name}_p05_ratio"] = float(
                ratios.quantile(0.05)
            )
            row[f"{name}_mean_time_ms"] = float(
                1000.0 * g[f"{name}_time_s"].mean()
            )
            row[f"{name}_mean_recourse"] = float(
                g[f"{name}_recourse"].mean()
            )
            row[f"{name}_attempts"] = int(
                g[f"{name}_refresh_attempt"].sum()
            )
            row[f"{name}_accepts"] = int(
                g[f"{name}_refresh_accept"].sum()
            )

        task_rows.append(row)

    task_summary = pd.DataFrame(task_rows)
    task_summary_path = OUTPUT_DIR / "safe_dynamic_task_summary.csv"
    task_summary.to_csv(task_summary_path, index=False)

    return all_path, summary_path, task_summary_path
...
def selftest() -> None:
    ...
    if not success or abs(value - 7.0) > 1e-8:
        raise AssertionError("Exact MILP self-test failed.")
    ...
    chosen, _, accepted, _ = safe_refresh_candidate(
        G=G,
        w=w,
        current=S,
        seed=456,
    ...
    if solution_weight(chosen, w) + 1e-12 < solution_weight(S, w):
        raise AssertionError(
            "Safe refresh degraded the current objective."
        )
...
def main() -> None:
    ...
    (OUTPUT_DIR / "safe_dynamic_checkpoints").mkdir(
        parents=True,
        exist_ok=True,
    )

    print("=" * 80)
    print("DYNAMIC MW2PS — SAFE-REFRESH EXACT CALIBRATION")
    print("=" * 80)
    ...
    pending = [
        t
        for t in tasks
        if not dynamic_checkpoint(t).exists()
    ]

    print(f"Total tasks  : {len(tasks)}")
    print(f"Completed    : {len(tasks) - len(pending)}")
    ...
    if pending:
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
            futures = {
                ex.submit(run_dynamic_task, task): task
                for task in pending
            }
    ...
    all_path, summary_path, task_summary_path = summarize()

    print()
    print("=" * 80)
    print("SAFE-REFRESH CALIBRATION COMPLETE")
    print("=" * 80)
